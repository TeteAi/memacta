...
test.describe("Soul Cinema", () => {
  ...
  test("10. sidebar Studio section has Soul Cinema link", async ({ page }) => {
    await page.goto("/tools/soul-cinema");
    // On desktop viewports the sidebar is always visible and Studio section
    // auto-expands because the current path matches one of its items.
    // Use a wider viewport to ensure the sidebar is rendered.
    await page.setViewportSize({ width: 1440, height: 900 });
    await page.goto("/tools/soul-cinema");
    // Expand Studio section if collapsed
    const studioBtn = page.locator("button").filter({ hasText: "Studio" }).first();
    const studioCount = await studioBtn.count();
    if (studioCount > 0) {
      const isExpanded = await page.locator('a[href="/tools/soul-cinema"]').count();
      if (isExpanded === 0) {
        await studioBtn.click();
      }
    }
    const soulCinemaLink = page.locator('a[href="/tools/soul-cinema"]').first();
    await expect(soulCinemaLink).toBeAttached();
    // Verify the href is correct (link exists in DOM, may be within fixed sidebar)
    const href = await soulCinemaLink.getAttribute("href");
    expect(href).toBe("/tools/soul-cinema");
  });
